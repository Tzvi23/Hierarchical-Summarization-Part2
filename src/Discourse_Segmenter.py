...
# Tzvi's functions
def find_index_to_remove(current_text, error_string):
    for index in range(len(current_text)):
        temp_string = current_text[index].lower().replace('\n', '').replace('\t', ' ')
        temp_string = temp_string.replace(' ', '')
        # temp_string = re.sub('[\s]{2,}', ' ', temp_string)
        error_temp_string = error_string.lower().replace('\n', '').replace('\t', ' ')
        error_temp_string = error_temp_string.replace(' ', '')
        # error_temp_string = re.sub('[\s]{2,}', ' ', error_temp_string)

        if temp_string.replace('.', '').strip() == error_temp_string.replace('.', '').strip():
            return index
    return -1
...
def do_preprocess(infile):
    ...
    chunked_pos = subprocess.check_output(
        ['java', '-Xmx4000m', '-classpath', java_classpath, 'edu.illinois.cs.cogcomp.lbj.chunk.ChunksAndPOSTags',
         infile])
    ...
    with open('errorlog.txt', mode='w') as errorLog:
        ...
        ch_parsed = subprocess.check_output([CHP, "tmp.tok"], stderr=errorLog)

    # Handling parsing errors that might effect the result
    # Writing the warning the parse.sh creates to file errorlog.txt
    # If the file is empty continuing the process else finds the sentence index and deletes
    # Then signals the main function to start the process again until no warnings found
    with open('errorlog.txt', mode='r') as errorLog:
        text = errorLog.readlines()
        if not len(text) == 0:  # If error found
            process_error_text = text[0].lower().split(' ')
    ...
    return ["tmp.tok", "tmp.tag", "tmp.chp"], True
...
def apply_model(test_f):
    ...
    # clf.raw_coef_ = np.delete(clf.raw_coef_, 265).reshape(1, 265)
    # clf.coef_ = clf.raw_coef_
    # clf.intercept_ = 0
...
def ruleBasedFix(output):
    ...
    for k in range(len(lines)):
        ...
        if k + 1 < len(lines):
            ...
            elif re.search("-- ", lines[k]) and (not re.search("\<S\>$", lines[k])) and re.search("^(-- )(.+)",
                                                                                                  lines[k + 1]):
                m = re.search("^(-- )(.+)", lines[k + 1])
                lines[k] += m.group(1)
                lines[k + 1] = m.group(2)
            ...
            elif (not re.search("\<S\>$", lines[k])) and (
                    re.search("^--( *)$", lines[k + 1]) or re.search("^-- \<S\>$", lines[k + 1])):
                lines[k] += lines[k + 1]
                skip = 1
